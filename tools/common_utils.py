...
#     max_side = np.maximum(h,w)
    max_side_w = np.maximum(h*2,w)
    max_side_h = max_side_w * 0.5
    square_bbox[:, 0] = bbox[:, 0] + w*0.5 - max_side_w *0.5
    square_bbox[:, 1] = bbox[:, 1] + h*0.5 - max_side_h *0.5
    square_bbox[:, 2] = square_bbox[:, 0] + max_side_w - 1
    square_bbox[:, 3] = square_bbox[:, 1] + max_side_h - 1
    return square_bbox
...
def getBboxLandmarkFromTxt(txt, with_landmark=True):
    ...
    for line in open(txt, 'r'): 
        ...
        img_path = components[0] ##os.path.join(dirname, components[0]) # file path
        if len(components) == 1: 
            yield  (img_path, None, None)
            continue
