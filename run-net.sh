...
if [[ "$1" == "pnet" ]] ; then
	...
	python training/train.py --stage=pnet --epoch ${epoch} --gpus 1,2
fi
# 3. stage: R-Net
...
if [[ "$1" == "rnet" ]]; then
	...
	python training/train.py --stage=rnet  --epoch ${epoch} --gpus 1,2
fi
...
if [[ "$1" == "onet" ]]; then
	...
	python training/train.py --stage=onet --epoch ${epoch} --gpus 1,2
fi
# 5. Done
